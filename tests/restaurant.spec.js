...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    const object = {comidas: {'batata frita': 10.90, 'croquete': 18.90}, bebidas: {'água': 2.50, 'refrigerante': 4.00, 'suco': 4.50}}
    const objetoRetornado = createMenu(object);
    expect(objetoRetornado.fetchMenu()).toEqual(object);
    ...
    expect(objetoRetornado.consumption).toEqual(['batata frita'])
    ...
    // ```
    objetoRetornado.consumption = []; // Limpando o array de consumo
    objetoRetornado.order('batata frita');
    objetoRetornado.order('croquete');
    objetoRetornado.order('água');
    objetoRetornado.order('suco');
    expect(objetoRetornado.consumption).toEqual(['batata frita','croquete','água','suco']);
    ...
    objetoRetornado.order('croquete');
    objetoRetornado.order('batata frita');
    expect(objetoRetornado.consumption).toEqual(['batata frita','croquete','batata frita']);
    ...
    // ```
    expect(objetoRetornado.pay()).toEqual(40.70)
